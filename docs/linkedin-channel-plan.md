...
| L2 | Routine 6 (LinkedIn) and its own tools: `linkedin_card` (what the lead needs now, every channel's history, rules, product voice and facts, ideas they have not had, results by idea on LinkedIn), `pick_linkedin` (invite or skip, with the reason), `plan_linkedin` (the next one or two messages with their words, checked against the rules and queued), `answer_linkedin` (a reply in the same conversation, sent outside the morning hours). The engine asks once per lead when it needs a pick, a plan (accepted, or 96 hours without an answer) or an answer, and ends the lead after 3 unanswered messages. A campaign opts in with `linkedin.planner: "claude"`; the email plan, compose and playbook paths skip those campaigns | built 2026-09-18; trigger not yet created |
