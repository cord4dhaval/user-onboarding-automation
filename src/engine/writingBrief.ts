...
export async function writingBriefFor(input: {
  ...
  return {
    ...
    rules: [
      ...
      "Where most of their work is away from a computer (site visits, field work), say the limit plainly in one short line. The privacy line only in the no_watching email or to someone who asked.",
      ...
      "Write in parts, not one block: subject (the point of this mail in 3 to 5 talking words), opening (the problem and its cost, one line under 90 characters, shown bold), scene (1 or 2 short lines, at most two **bold** figures), reveal (1 or 2 lines on what TeamGrid does, as a result), question (on a link ask the price from writing.facts.plans, shown bold in a box; on a reply ask the question), ps, plus the timeline or reply_options your layout_tests arm asks for. No shows list and no cost_lines box in hot or warm emails: the numbers go in the bold lines.",
      ...
      "The subject is the point of this mail said in the fewest talking words: 3 to 5 words, 18 to 45 characters. No figures, no question mark, no colon. Words a shop owner says out loud, never words people only write (no payroll, attendance, timesheet, overtime, idle, pipeline, productivity, capacity, visibility, bottleneck, loaded). \"what your showroom staff did today\", not \"₹88,000 a month a punch machine will not catch\".",
      "The company name goes in the subject wherever we hold a real one and it fits the line naturally: \"Salary day at Sree Motors\", \"Who came in at Aventurine Homes\". Take it from context.site_title trimmed to the name itself (a title like \"Top Property Consultant & Mandate Services in Mumbai\" holds no name), from what their site text calls itself, or from the domain read as words (sreemotors.com is Sree Motors). Skip the name when you are not sure what they are called, when the domain is mangled (wwwpioneercars.in, abc.com), or when the line only fits with it forced in — \"your showroom\" is better than a wrong name. Never the raw domain, never a name with www or a dot in it, and never their first name.",
      "Do not put the name in two mails in a row to the same person. Once it has been said, it is theirs and the next subject speaks about the work.",
