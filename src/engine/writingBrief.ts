...
export async function writingBriefFor(input: {
  ...
  return {
    ...
    rules: [
      ...
      "The company name belongs in the subject only when the mail itself is about their company, and only when the name is a clean name from their site, never one built from a domain. A mail about a problem every office has carries no name: a name there reads as a mail merge. Their first name never goes in the subject.",
