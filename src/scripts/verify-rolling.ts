...
console.log("idea bank");
{
  ...
  const { ideaLimits, capFor } = await import("../engine/ideas");
  check("the cap holds while an idea under it is open", capFor(9, new Map([[1, 9], [2, 3]]), [1, 2]) === 9);
  check("every open idea at the cap still leaves the least-used one", capFor(9, new Map([[1, 9], [2, 11]]), [1, 2]) === 10);
